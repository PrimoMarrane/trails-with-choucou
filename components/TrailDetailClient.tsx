...
import type { TrailDetail } from '@/lib/data';
...
export default function TrailDetailClient({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Refresh server data
      router.refresh();
      // Also optimistically update local state so user sees changes immediately
      const updated = await response.json();
      setTrail((prev) => ({ ...prev, ...updated.trail }));
      setIsEditing(false);
      setEditForm(emptyEditForm);
    } catch (error) {
          ...
          <div className="bg-white rounded-lg shadow-md p-6 max-w-2xl space-y-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setEditForm((f) => ({ ...f, difficulty: e.target.value }))
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditForm((f) => ({ ...f, dateCompleted: e.target.value }))
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
              />
            </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-lg shadow-md overflow-hidden">
                <div key={`trail-map-${trail.id}`} className="h-96">
                  <MapView
                    selectedTrail={{ trail, trackPoints: trail.trackPoints }}
                  />
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold text-gray-900">
                      {trail.distanceKm != null
                        ? `${Number(trail.distanceKm).toFixed(2)} km`
                        : '—'}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-lg font-semibold text-gray-900">
                      {trail.elevationGainM != null
                        ? `${Number(trail.elevationGainM).toFixed(0)} m`
                        : '—'}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-lg font-semibold text-gray-900">
                      {trail.elevationLossM != null
                        ? `${Number(trail.elevationLossM).toFixed(0)} m`
                        : '—'}
                    </p>
                  </div>
                  <div>
